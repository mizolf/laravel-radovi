                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('tasks.update', $task) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="title_en" id="title_en" value="{{ old('title_en', $task->title_en) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <select name="study_type" id="study_type"
                                ...
                                <option value="strucni" {{ old('study_type', $task->study_type) === 'strucni' ? 'selected' : '' }}>{{ __('tasks.strucni') }}</option>
                                <option value="preddiplomski" {{ old('study_type', $task->study_type) === 'preddiplomski' ? 'selected' : '' }}>{{ __('tasks.preddiplomski') }}</option>
                                <option value="diplomski" {{ old('study_type', $task->study_type) === 'diplomski' ? 'selected' : '' }}>{{ __('tasks.diplomski') }}</option>
                            </select>
                            @error('study_type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
